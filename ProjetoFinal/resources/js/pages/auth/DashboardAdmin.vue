...
<template>
    <section class="relative min-h-screen overflow-hidden p-4 md:p-10 bg-blue-100">
        <div class="bg-white p-4 md:p-5 rounded-2xl shadow-md flex justify-between items-center mb-6 md:mb-10">
            ...
            <div class="flex items-center gap-2 md:gap-4">
                <div class="hidden sm:flex items-center gap-2 px-4 py-2 rounded-xl border border-gray-200 text-sm font-bold text-gray-500">
                    <i class="fa-solid fa-circle-user text-lg text-grey-400"></i> Administrador
                </div>
                <a @click.prevent="logout" href="/login" class="bg-red-500 p-2 px-4 md:px-6 rounded-xl text-sm font-bold text-white hover:bg-red-600 transition shadow-md">
                    <i class="fa-solid fa-arrow-right-from-bracket"></i> <span class="hidden sm:inline">Sair</span>
                </a>
            </div>
        </div>
        ...
        <div class="flex flex-col gap-6 md:gap-10">
            <div class="w-full flex flex-col md:flex-row gap-6 md:gap-10">
                ...
                <div class="flex-1 bg-white p-6 md:p-10 rounded-3xl shadow-lg relative">
                    <div class="flex items-center mb-6 mt-2 gap-4">
                        <label class="p-4 rounded-2xl shadow-lg bg-blue-100">
                            <i class="fa-solid fa-question text-blue-600"></i>
                        </label>
                        <p class="font-bold text-xl md:text-2xl text-black">Criar Nova Pergunta</p>
                    </div>
                    <form class="flex flex-col gap-3" @submit.prevent="submitQuestion">
                        ...
                        <p v-show="errorMessage" class="text-red-500 text-sm h-5">{{ errorMessage }}</p>
                ...
                <div class="flex-1 bg-white p-6 md:p-10 rounded-3xl shadow-lg relative">
                    <div class="flex items-center mb-6 mt-2 gap-4">
                        ...
                        <p class="font-bold text-xl md:text-2xl text-black">Estatísticas</p>
                    </div>
                    <div class="flex gap-4">
                        <div class="flex-1 bg-gradient-to-br from-blue-500 to-blue-600 p-4 md:p-6 rounded-2xl text-white">
                            <p class="text-sm opacity-80">Total de Perguntas</p>
                            <p class="text-3xl md:text-4xl font-bold mt-2">{{ totalQuestions }}</p>
                        </div>
                        <div class="flex-1 bg-gradient-to-br from-blue-500 to-blue-600 p-4 md:p-6 rounded-2xl text-white">
                            <p class="text-sm opacity-80">Categorias</p>
                            <p class="text-3xl md:text-4xl font-bold mt-2">{{ totalCategories }}</p>
                        </div>
                    </div>
                    ...
                    <div  v-else class="flex flex-col gap-2 mt-4 overflow-y-auto max-h-64">
